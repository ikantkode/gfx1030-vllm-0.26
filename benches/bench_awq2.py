"""Sweep 2: long-K AWQ decode shape with SPLIT_K variants (gfx1030)."""
...
# kernel identical to bench_awq.py (SPLIT_K support via pid_z + output offset)
@triton.jit
def awq_gemm_kernel(
...
def bench(m, k, BM, BN, BK, W, S, SK, g=128):
    ...
    if SK == 1:
        ...
        out = torch.zeros((SK, M, N), dtype=torch.float16, device="cuda")
    ...
    try:
        ...
        for _ in range(200):
            call()
        r = out.sum(0) if SK > 1 else out
        torch.cuda.synchronize()
        us = (time.perf_counter() - t) / 200 * 1e6
        wbytes = k * m * 0.5 + k * m / g * 2 + k * m / (8 * g) * 4
        floor = wbytes / 400e9 * 1e6
        print(f"m={m:5d} k={k:5d} BM={BM:3d} BN={BN:3d} BK={BK:3d} W={W} S={S} SK={SK}: "
              f"{us:7.1f} us ({floor / us * 100:4.0f}% peak)", flush=True)
    except Exception as e:
        print(f"m={m} k={k} BM={BM} BN={BN} BK={BK} W={W} S={S} SK={SK}: FAIL {type(e).__name__} {str(e)[:70]}", flush=True)


# long-K down_proj shape (current best: 16/128/64/8/3/SK1 = 220us, 14%)
for cfg in [
    (16, 128, 64, 8, 3, 1),   # current
    (16, 128, 64, 8, 3, 2),
    (16, 128, 64, 8, 3, 4),
    (16, 128, 64, 8, 3, 8),
    (16, 64, 64, 4, 3, 4),
    ...
    bench(2560, 9216, *cfg)
print("--- gate/up shape recheck with best SK ---", flush=True)
for cfg in [(16, 128, 64, 8, 3, 1), (16, 128, 64, 8, 3, 2)]:
    bench(9216, 2560, *cfg)
